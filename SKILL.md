...
| Markdown 审核报告 | `<报告文件名>.review.md` | HTML 的源文件，便于编辑与归档 |
...
5. **输出审核报告**：
   ...
   b. 运行 `python scripts/make_html_report.py <上一步的 .review.md>` 生成 `<报告文件名>.review.html`；
...
3. 每份产出 `.workpaper.md` + `.review.md` + `.review.html`，全部存到被审核文件夹内（用户另有指定除外）；对话内只报该份的总体评价与严重发现摘要。
4. 全部完成后，按 `references/batch-summary-template.md` 写 `batch-review-summary.md`，并运行 `make_html_report.py` 生成 `batch-review-summary.html`，在对话内展示"各报告结论一览"与"严重问题清单"两节。
