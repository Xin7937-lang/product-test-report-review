...
### 严重
| # | 类别 | 位置 | 问题描述 | 预期 | 实际 | 证据摘录 | 置信度 | 建议 |
|---|---|---|---|---|---|---|---|---|
| 1 | DC-G02 | [P0035]/[T02] | 汇总结论与单项数据矛盾 | 单项超限时不应判合格 | 表中实测超限但汇总写“通过” | “实测 38.2mΩ（限值≤35mΩ）…判定：合格” | 高 | 复核该项判定，必要时改判并触发整改流程 |
...
| # | 预期对象 | 实际对象/位置 | 审阅方式 | 一致性 | 备注 |
|---|---|---|---|---|---|
| 1 | 正文称“见图3振动后外观” | [S08] 图片页；无可机读正文说明 | manual | ⚠️ 待人工核对 | 当前环境无视觉能力，需人工核对图片内容 |
| 2 | 结论页称“详见图5曲线” | [P0211]/[S12] | vision | ❌ 不一致 | 曲线显示峰值超限，正文仍写“无异常” |
...
| # | 事项 | 位置 | 原因 | 当前证据 |
|---|---|---|---|---|
| 1 | 第 5 页截图内容 | [S05] | no-vision/manual fallback；图片内文字不可机读 | `extract_report.py` 标记为图片页 |
| 2 | 校准证书真实性 | [P0042] | 文本仅声明“有效期至 2026-05”，需核证书原件 | “校准有效期至2026年5月” |
